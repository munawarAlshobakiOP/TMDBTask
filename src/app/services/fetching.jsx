...
export async function FetchingDetails(mediaType, mediaId) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        `Media fetch failed: ${res.status} - ${errorData.status_message || 'Unknown error'}`
      );
    }
...
export async function FetchingCast(mediaType, mediaId) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const errorData = await res.json().catch(() => ({}));
      throw new Error(
        `Cast fetch failed: ${res.status} - ${errorData.status_message || 'Unknown error'}`
      );
    }
...
export async function fetchLanguages() {
  try {
    const res = await fetch(
      `${baseurl}/configuration/languages?api_key=${API_KEY}`,
      {
        method: 'GET',
        headers: {
          accept: 'application/json',
...
export const fetchRandomImg = async (setBackgroundImage, setIsLoading) => {
  try {
    ...
    if (moviesWithBackdrops.length > 0) {
      ...
      setBackgroundImage(
        `${IMAGE_BASE}/original/7WJjFviFBffEJvkAms4uWwbcVUk.jpg`
      );
    }
  } catch (error) {
    setBackgroundImage(
...
export async function fetchSearchResults(query, page = 1) {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const errorData = await res.json().catch(() => ({}));
      throw new Error(
        `Search failed: ${res.status} - ${errorData.status_message || 'Unknown error'}`
      );
    }
